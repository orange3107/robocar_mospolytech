...
void PCLLocalization::initializePubSub()
{
  ...
  pub_pose_stamped_ = create_publisher<geometry_msgs::msg::PoseStamped>(
    "initial_pose",
    rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());
...
void PCLLocalization::cloudReceived(const sensor_msgs::msg::PointCloud2::ConstSharedPtr msg)
{
  ...
  last_pose_ = *corrent_pose_with_cov_stamped_ptr_;
  
  geometry_msgs::msg::PoseStamped transform_map_to_base_link;

 try
    {

      transform_map_to_base_link.header.stamp = this->get_clock()->now();
      transform_map_to_base_link.header.frame_id = "map";

      transform_map_to_base_link.pose.position.x = last_pose_.pose.pose.position.x;
      transform_map_to_base_link.pose.position.y = last_pose_.pose.pose.position.y;
      transform_map_to_base_link.pose.position.z = last_pose_.pose.pose.position.z;

      transform_map_to_base_link.pose.orientation = last_pose_.pose.pose.orientation;
      ...
      if(!stop_timer && (this->get_clock()->now() - timer).seconds() > 10.0){
        stop_timer = true;
        pub_pose_stamped_->publish(transform_map_to_base_link);
        rclcpp::shutdown();
      }
    }
